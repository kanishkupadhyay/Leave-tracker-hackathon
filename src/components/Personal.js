import React, { useState } from 'react';

export default function Personal(props) {

   const [userData] = useState(localStorage.getItem('userInfo'))
   let userObj = JSON.parse(userData)

   return (
      <>
         <div className="container">
            <div className="svg-background"></div>
            <div className="svg-background2"></div>
            <div className="circle"></div>
            <img className="menu-icon" src="https://pngimage.net/wp-content/uploads/2018/06/white-menu-icon-png-8.png" />
            <img className="profile-img" src="data:image/png;base64," />
            <div className="text-container">
               <p className="title-text">{userObj[0].firstName + ' ' + userObj[0].lastName}</p>
               <p className="info-text">{props.employeeRole}</p>
               <p className="desc-text" title={userObj[0].email}>Email: {userObj[0].email}</p>
               <p className="desc-text" title={userObj[0].phone}>Phone: {userObj[0].phone}</p>
               <p className="desc-text" title={userObj[0].leave}>Leaves: {userObj[0].quantity}</p>
            </div>
         </div>

      </>
   )
}

Personal.defaultProps = {
   employeeRole: 'Software Developer'
}